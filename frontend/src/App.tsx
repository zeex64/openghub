...
import { api, AdvancedSettings, ButtonPayload, Choices, DeviceState, DeviceSummary, DPIStage, Haptics, LightingEffect, onDevicesUpdate, onDeviceUpdate, Profile } from './backend'
...
type Page = 'dashboard' | 'dpi' | 'profiles' | 'buttons' | 'lighting' | 'haptics' | 'advanced'
...
function MouseArt({interactive = false, selected, onSelect, visibleMarkers=[0,1,2], actuation, mappingControls=false, mappingAssignments=[],modelId='superstrike'}: {interactive?: boolean; selected?: number; onSelect?: (i: number) => void; visibleMarkers?:number[];actuation?:{values:number[];max:number;haptics:number[];maxHaptics:number};mappingControls?:boolean;mappingAssignments?:string[];modelId?:string}) {
  ...
  const g502=modelId==='g502-se-hero'
  return <div className={`mouse-art ${interactive ? 'interactive' : ''}`} role="img" aria-label={g502?'G502 HERO mouse':'PRO X2 Superstrike mouse'}>
    <img src={g502?g502TopImage:productImage} alt="" draggable="false"/>
    {!g502&&actuation&&<ActuationOverlay values={actuation.values} max={actuation.max} haptics={actuation.haptics} maxHaptics={actuation.maxHaptics} selected={selected} onSelect={onSelect}/>}
    {!g502&&mappingControls&&<TopControlOverlay selected={selected} onSelect={onSelect} assignments={mappingAssignments}/>}
...
function Dashboard({state, notify, active=true}: {state: DeviceState; notify: (s: string, e?: boolean) => void; active?:boolean}) {
  const [selectedRates,setSelectedRates] = useState({wired:0,wireless:0})
  const [savingRate, setSavingRate] = useState(false)
  const [operation,setOperation] = useState<OperationState>()
  const supportedRates=state.capabilities.pollingRates.length?state.capabilities.pollingRates:rates
  const shownTransports:('wired'|'wireless')[]=state.capabilities.separatePollingRates?['wireless','wired']:[state.connectionType]
  useEffect(() => {
  ...
  return <div className={`hub-dashboard page-enter ${state.modelId==='g502-se-hero'?'g502-dashboard':''}`}>
    <section className={`hub-panel three-product-view ${state.modelId==='g502-se-hero'?'g502-product-view':''}`}><Suspense fallback={<Loader text="Preparing 3D view"/>}><ThreeMouseViewer active={active} modelId={state.modelId}/></Suspense></section>
    <section className="hub-panel rate-overview"><div className="rate-overview-copy"><h2>{state.capabilities.separatePollingRates?'Saved polling rates':'Report rate'}</h2><p>The selected rate is applied live and saved to the current onboard profile.</p><InlineStatus state={operation} idle={`Live measurement: ${state.pollingRate?`${state.pollingRate} Hz`:'waiting for movement'}`}/></div><div className="transport-rate-list">{shownTransports.map(transport=><section key={transport} className={`transport-rate-row ${state.connectionType===transport?'connected':'inactive'}`}><header><div><span>{state.capabilities.separatePollingRates?transport.toUpperCase():'SAVED SETTING'}</span><strong>{selectedRates[transport]?`${selectedRates[transport]} Hz`:'Not saved'}</strong></div><small>{state.connectionType===transport?'Current connection':`Connect over ${transport} to change`}</small></header><div className={`rate-track ${savingRate?'saving':''}`}>{supportedRates.slice().reverse().map(rate=><button key={rate} className={selectedRates[transport]===rate?'active':''} onClick={()=>chooseRate(transport,rate)} disabled={savingRate||state.connectionType!==transport}>{rate}<small>Hz</small></button>)}</div></section>)}</div></section>
  </div>
}

function DPIPage({notify,capabilities}: {notify: (s: string, e?: boolean) => void;capabilities:DeviceState['capabilities']}) {
  ...
  const dpiMin=capabilities.dpiMin||100,dpiMax=capabilities.dpiMax||44000,dpiStep=capabilities.dpiStep||50
  const preview=async(stage:DPIStage)=>{if(!activeProfile||saving||!stage.enabled)return;setSaving(true);setSelected(stage.index);setOperation({text:`Applying slot ${stage.index+1}…`,tone:'saving'});try{await api.selectDPI(activeProfile.sector,stage.index,Math.max(dpiMin,Math.min(dpiMax,stage.x)),stage.lod);setActiveProfile(current=>current?{...current,currentDpiStage:stage.index,dpiX:stage.x,dpiY:stage.y}:current);setOperation({text:`Slot ${stage.index+1} is active`,tone:'saved'})}catch(error){setOperation({text:'Could not select the DPI slot',tone:'error'});notify(String(error),true)}finally{setSaving(false)}}
  const toggle=(stage:DPIStage)=>{if(stage.enabled&&drafts.filter(item=>item.enabled).length===1)return;const enabled=!stage.enabled;updateStage(stage.index,{enabled});if(!enabled){const replacement=drafts.find(item=>item.index!==stage.index&&item.enabled);if(defaultStage===stage.index&&replacement)setDefaultStage(replacement.index);if(selected===stage.index&&replacement)setSelected(replacement.index);if(shiftStage===stage.index&&replacement)setShiftStage(replacement.index);if(activeProfile?.currentDpiStage===stage.index&&replacement)setActiveProfile(current=>current?{...current,currentDpiStage:replacement.index}:current)}}
  const saveProfile=async()=>{if(!activeProfile||saving||!dirty)return;setSaving(true);setOperation({text:'Saving DPI settings to the current profile…',tone:'saving'});try{const normalized=drafts.map(stage=>({...stage,x:Math.max(dpiMin,Math.min(dpiMax,stage.x)),y:Math.max(dpiMin,Math.min(dpiMax,stage.y))}));const activeStage=normalized.some(stage=>stage.index===activeProfile.currentDpiStage&&stage.enabled)?activeProfile.currentDpiStage:defaultStage;const actualSector=await api.saveDPIToProfile(activeProfile.sector,normalized,defaultStage,activeStage,shiftStage);setDrafts(normalized);setActiveProfile(current=>current?{...current,sector:actualSector,dpiStages:normalized,currentDpiStage:activeStage,defaultDpiStage:defaultStage,shiftDpiStage:shiftStage}:current);setDirty(false);setOperation({text:'DPI settings saved to the current profile',tone:'saved'});notify('DPI settings saved to the current profile')}catch(error){setOperation({text:'DPI profile save failed',tone:'error'});notify(String(error),true)}finally{setSaving(false)}}
  ...
  const lodAvailable=capabilities.dpiLiftOff&&Boolean(surface?.gamingSurfaceAvailable)&&surface?.gamingSurfaceMode!==4
  ...
  return <div className="dpi-hub-page page-enter">
    <section className="dpi-stage-panel">
      ...
      <div className="dpi-stage-overview">{drafts.map(stage=><article key={stage.index} aria-label={`DPI slot ${stage.index+1}`} className={`dpi-stage-card ${selected===stage.index?'selected':''} ${!stage.enabled?'disabled':''}`} onClick={()=>setSelected(stage.index)}>
        ...
        <span className="dpi-slot-badges">{activeProfile.currentDpiStage===stage.index&&<em className="active">Active</em>}{defaultStage===stage.index&&<em>Default</em>}{!capabilities.dpiLiftOff&&shiftStage===stage.index&&<em>Shift</em>}</span>
        <i className={`dpi-slot-state ${stage.enabled?'on':''}`}/>
        <div className="dpi-card-sensitivity"><input className="range" aria-label={`Slot ${stage.index+1} DPI slider`} type="range" min={dpiMin} max={dpiMax} step={dpiStep} disabled={!stage.enabled||saving} value={stage.x} onChange={event=>updateStage(stage.index,{x:+event.target.value,y:+event.target.value})}/><label><input aria-label={`Slot ${stage.index+1} DPI`} type="number" min={dpiMin} max={dpiMax} step={dpiStep} disabled={!stage.enabled||saving} value={stage.x} onChange={event=>updateStage(stage.index,{x:+event.target.value,y:+event.target.value})}/><span>DPI</span></label></div>
      </article>)}</div>
    ...
    <aside className="dpi-editor-panel">
      ...
      <div className="dpi-editor-body">
        ...
        {capabilities.dpiLiftOff&&<section className={`dpi-editor-section ${!selectedStage.enabled?'control-disabled':''}`}><div className="dpi-setting-heading"><span>SENSOR</span><h3>Lift-off distance</h3><p>{lodAvailable?'Tracking cutoff when the mouse is lifted.':'Available when Gaming Surface is Auto or On.'}</p></div><div className="mapping-category-tabs dpi-lod-tabs">{[{value:1,label:'Low'},{value:2,label:'Medium'},{value:3,label:'High'}].map(option=><button key={option.value} type="button" className={selectedStage.lod===option.value?'selected':''} disabled={!selectedStage.enabled||saving||!lodAvailable} onClick={()=>updateStage(selectedStage.index,{lod:option.value})}>{option.label}</button>)}</div></section>}
        <section className={`dpi-editor-section dpi-slot-actions ${!selectedStage.enabled?'control-disabled':''}`}><div className="dpi-setting-heading"><span>PROFILE BEHAVIOR</span><h3>Active and default DPI</h3><p>Activate this slot now or choose how the profile uses it.</p></div><div><button className="button subtle" disabled={!selectedStage.enabled||saving||activeProfile.currentDpiStage===selectedStage.index} onClick={()=>preview(selectedStage)}>{activeProfile.currentDpiStage===selectedStage.index?'Active now':'Activate slot'}</button><button className="button subtle" disabled={!selectedStage.enabled||saving||defaultStage===selectedStage.index} onClick={()=>chooseDefault(selectedStage)}>{defaultStage===selectedStage.index?'Profile default':'Set as default'}</button>{!capabilities.dpiLiftOff&&<button className="button subtle" disabled={!selectedStage.enabled||saving||shiftStage===selectedStage.index} onClick={()=>{setShiftStage(selectedStage.index);setDirty(true);setOperation({text:`Slot ${selectedStage.index+1} will be used for DPI Shift when saved`})}}>{shiftStage===selectedStage.index?'DPI Shift speed':'Set as DPI Shift'}</button>}</div></section>
      </div>
...
function Buttons({notify,modelId}: {notify: (s: string, e?: boolean) => void;modelId:string}) {
  ...
  return <div className="button-mapper page-enter">
    <section className="button-visual-panel">
      <div className="button-top-view"><MouseArt interactive mappingControls modelId={modelId} mappingAssignments={data.buttons.map(button=>button.description)} selected={selected} onSelect={index=>{if(data.buttons[index])setSelected(index)}}/></div>
      <div className="button-side-view">{modelId==='g502-se-hero'?<img className="mouse-side-art" src={g502SideImage} alt="G502 HERO side view"/>:<MouseSideArt interactive assignments={data.buttons.map(button=>button.description)} selected={selected} onSelect={index=>setSelected(index)}/>}</div>
    </section>
    <aside className="mapping-editor-panel">
      <header><div><span>SELECTED CONTROL</span><h1>{item.name}</h1><p>{item.description}</p>{modelId==='g502-se-hero'&&<select className="mapping-control-select" value={selected} onChange={event=>setSelected(+event.target.value)}>{data.buttons.map(button=><option key={button.index} value={button.index}>{button.index+1}. {button.name}</option>)}</select>}</div></header>
...
function LightingPage({notify}: {notify:(s:string,e?:boolean)=>void}) {
  const [profile,setProfile]=useState<Profile>(),[zone,setZone]=useState(0),[saving,setSaving]=useState(false),[operation,setOperation]=useState<OperationState>()
  const load=useCallback(async()=>{try{const list=await api.profiles(),active=list.find(item=>item.active)||list[0];setProfile(active)}catch(error){notify(String(error),true)}},[notify])
  useEffect(()=>{load()},[load])
  if(!profile)return <Loader text="Reading lighting profile"/>
  const fallback:LightingEffect={mode:1,red:0,green:170,blue:255,periodMs:5000,brightness:100}
  const storedEffect=profile.lighting?.[zone]
  const effect={...fallback,...storedEffect,periodMs:storedEffect?.periodMs||fallback.periodMs,brightness:storedEffect?.brightness||fallback.brightness}
  const update=(change:Partial<LightingEffect>)=>setProfile(current=>current?{...current,lighting:current.lighting.map((item,index)=>index===zone?{...effect,...change}:item)}:current)
  const color=`#${[effect.red,effect.green,effect.blue].map(value=>value.toString(16).padStart(2,'0')).join('')}`
  const save=async()=>{setSaving(true);setOperation({text:'Saving lighting to the current profile…',tone:'saving'});try{const sector=await api.setLighting(profile.sector,zone,effect);setProfile(current=>current?{...current,sector}:current);setOperation({text:'Lighting saved to the current profile',tone:'saved'});notify('Lighting saved')}catch(error){setOperation({text:'Lighting write failed',tone:'error'});notify(String(error),true)}finally{setSaving(false)}}
  return <div className="lighting-manager page-enter"><section className="lighting-preview-panel"><header><span>LIGHTSYNC</span><h1>Lighting</h1><p>Configure the G502 primary and logo zones.</p></header><img src={g502TopImage} alt="G502 HERO lighting preview"/><div className="lighting-glow" style={{background:color}}/></section><aside className="haptics-settings-panel lighting-control-panel"><header><div><span>ACTIVE PROFILE</span><h2>{friendlyName(profile.name,`Profile ${profile.index}`)}</h2><p>Effects are applied live and stored onboard.</p></div></header><div className="advanced-settings-list"><section className="advanced-setting-group"><div className="mapping-category-tabs advanced-mode-tabs">{['Primary','Logo'].map((name,index)=><button key={name} className={zone===index?'selected':''} onClick={()=>setZone(index)}>{name}</button>)}</div></section><section className="advanced-setting-group"><div className="advanced-setting-title"><span>EFFECT</span><h3>{zone===0?'Primary zone':'Logo zone'}</h3><p>{zone===0?'The three bars also indicate the active DPI slot; their color follows this effect.':'Lighting for the Logitech G logo.'}</p></div><div className="mapping-category-tabs advanced-mode-tabs">{[{mode:0,label:'Off'},{mode:1,label:'Fixed'},{mode:2,label:'Breathing'},{mode:3,label:'Cycle'}].map(option=><button key={option.mode} className={effect.mode===option.mode?'selected':''} onClick={()=>update({mode:option.mode})}>{option.label}</button>)}</div>{(effect.mode===1||effect.mode===2)&&<label className="lighting-color-row"><span>Color</span><input type="color" value={color} onChange={event=>{const value=parseInt(event.target.value.slice(1),16);update({red:value>>16,green:(value>>8)&255,blue:value&255})}}/></label>}{(effect.mode===2||effect.mode===3)&&<><Slider title="Effect rate" description="Animation cycle length" min={1000} max={15000} step={100} value={effect.periodMs||5000} onChange={value=>update({periodMs:value})}/><Slider title="Brightness" description="Lighting intensity" min={1} max={100} value={effect.brightness||100} onChange={value=>update({brightness:value})}/></>}</section></div><footer><InlineStatus state={operation} idle="Stored in the current onboard profile"/><button className="button primary" disabled={saving} onClick={save}>{saving?'Saving…':'Save lighting'}</button></footer></aside></div>
}
...
function AdvancedPage({notify,modelId}: {notify: (s: string, e?: boolean) => void;modelId:string}) {
  ...
  const run=async(label:string,work:()=>Promise<void>,apply:()=>void)=>{setSaving(true);setOperation({text:`Applying ${label.toLowerCase()}…`,tone:'saving'});try{await work();apply();setOperation({text:`${label} updated`,tone:'saved'});notify(`${label} updated`)}catch(error){setOperation({text:`Could not update ${label.toLowerCase()}`,tone:'error'});notify(String(error),true);load()}finally{setSaving(false)}}
  const setSurface=(mode:number)=>run('Gaming surface',()=>api.setGamingSurface(mode),()=>setData(current=>current?{...current,gamingSurfaceMode:mode}:current))
  const setBhop=(windowMs:number)=>run('Bhop mode',()=>api.setBhop(windowMs),()=>setData(current=>current?{...current,bhopKnown:true,bhopWindowMs:windowMs}:current))
  const bhopEnabled=data.bhopKnown&&data.bhopWindowMs>0
  return <div className="advanced-manager page-enter"><section className="advanced-device-panel"><header><span>DEVICE-SPECIFIC CONTROLS</span><h1>Advanced</h1><p>{modelId==='g502-se-hero'?'Startup and DPI indicator behavior for the G502 HERO.':'Sensor and scroll-wheel behavior for the PRO X2 Superstrike.'}</p></header><div className="advanced-device-art"><MouseArt modelId={modelId}/></div></section><aside className="haptics-settings-panel advanced-control-panel"><header><div><span>{modelId==='g502-se-hero'?'G502 HERO':'SUPERSTRIKE'}</span><h2>Device behavior</h2><p>Controls exposed by this mouse and firmware.</p></div></header><div className="advanced-settings-list">
    {data.gamingSurfaceAvailable&&<section className="advanced-setting-group"><div className="advanced-setting-title"><span>SENSOR</span><h3>Gaming surface mode</h3><p>Choose how the sensor adapts its tracking.</p></div><div className="mapping-category-tabs advanced-mode-tabs">{[{mode:0,label:'Auto'},{mode:2,label:'On'},{mode:4,label:'Off'}].map(option=><button key={option.mode} disabled={saving} className={data.gamingSurfaceMode===option.mode?'selected':''} onClick={()=>setSurface(option.mode)}>{option.label}</button>)}</div></section>}
    {data.bhopAvailable&&<section className="advanced-setting-group bhop-setting-group"><div className="profile-setting-line advanced-toggle-line"><div><span>SCROLL WHEEL</span><h3>Bhop mode</h3><p>{bhopEnabled?'Require two wheel events inside the detection window.':'Use normal scroll-wheel behavior.'}</p></div><button disabled={saving||!data.bhopKnown} className={`switch ${bhopEnabled?'on':''}`} onClick={()=>setBhop(bhopEnabled?0:bhopWindow)}><i/></button></div><div className={!bhopEnabled?'advanced-disabled-control':''}><Slider title="Detection window" description="Maximum time allowed between wheel events" min={100} max={1000} step={10} value={bhopEnabled?data.bhopWindowMs:bhopWindow} onChange={setBhopWindow} onCommit={value=>{setBhopWindow(value);if(bhopEnabled)setBhop(value)}}/></div></section>}
    {data.startupEffectAvailable&&<section className="advanced-setting-group"><div className="profile-setting-line advanced-toggle-line"><div><span>LIGHTING</span><h3>Device startup effect</h3><p>Play the lighting animation when the mouse starts.</p></div><button disabled={saving} className={`switch ${data.startupEffectEnabled?'on':''}`} onClick={()=>run('Startup effect',()=>api.setStartupEffect(!data.startupEffectEnabled),()=>setData(current=>current?{...current,startupEffectEnabled:!data.startupEffectEnabled}:current))}><i/></button></div></section>}
    {data.dpiLightingAvailable&&<section className="advanced-setting-group"><div className="profile-setting-line advanced-toggle-line"><div><span>DPI INDICATOR</span><h3>DPI lighting</h3><p>Keep the three DPI indicator LEDs enabled.</p></div><button disabled={saving} className={`switch ${data.dpiLightingEnabled?'on':''}`} onClick={()=>run('DPI lighting',()=>api.setDPILighting(!data.dpiLightingEnabled),()=>setData(current=>current?{...current,dpiLightingEnabled:!data.dpiLightingEnabled}:current))}><i/></button></div></section>}
  </div><footer><div><InlineStatus state={operation} idle="Changes apply directly to the mouse"/></div></footer></aside></div>
}
...
export default function App() {
  const emptyCapabilities={battery:false,dpi:false,dpiStages:0,dpiMin:100,dpiMax:44000,dpiStep:50,dpiLiftOff:false,pollingRates:[],separatePollingRates:false,profiles:false,onboardMode:false,buttonMapping:false,haptics:false,gamingSurface:false,bhop:false,lighting:false,startupEffect:false,dpiLighting:false}
  const [page,setPage] = useState<Page>(()=>{const saved=window.sessionStorage?.getItem('openghub-page');return ['dashboard','dpi','profiles','buttons','lighting','haptics','advanced'].includes(saved||'')?saved as Page:'dashboard'}), [state,setState] = useState<DeviceState>({connected:false,permissionDenied:false,name:'',path:'',battery:0,charging:false,hasBattery:false,profile:'',dpiX:0,dpiY:0,pollingRate:0,configuredPollingRate:0,connectionType:'wired',wiredPollingRate:0,wirelessPollingRate:0,onboardModeAvailable:false,onboardModeEnabled:false,deviceId:'',modelId:'',capabilities:emptyCapabilities}), [devices,setDevices] = useState<DeviceSummary[]>([]), [toasts,setToasts] = useState<Toast[]>([]), [showDeviceLibrary,setShowDeviceLibrary] = useState(true)
  ...
  useEffect(()=>{window.sessionStorage?.setItem('openghub-page',page)},[page])
  ...
  const nav = useMemo(() => {
    ...
    return [
      ...
      {id:'lighting' as Page,label:'Lighting',visible:capabilities.lighting},
      {id:'haptics' as Page,label:'Haptics',visible:capabilities.haptics},
      {id:'advanced' as Page,label:'Advanced',visible:capabilities.gamingSurface||capabilities.bhop||capabilities.startupEffect||capabilities.dpiLighting},
    ].filter(item=>item.visible)
  },[state.capabilities])
  const view = page === 'dpi' ? <DPIPage notify={notify} capabilities={state.capabilities}/> : page === 'profiles' ? <ProfilesPage notify={notify} state={state} onModeChanged={enabled=>setState(current=>({...current,onboardModeEnabled:enabled}))}/> : page === 'buttons' ? <Buttons notify={notify} modelId={state.modelId}/> : page === 'lighting'?<LightingPage notify={notify}/> : page === 'haptics' ? <HapticsPage notify={notify}/> : <AdvancedPage notify={notify} modelId={state.modelId}/>
  ...
  return <div className="hub-shell">
    <section className={`hub-workspace ${libraryActive?'library-mode':''}`}>
      ...
      <main className="content">
        {libraryActive?<DeviceLibrary state={state} connectedDevices={devices} onOpen={openDevice}/>:<><div className={`persistent-home ${page === 'dashboard' ? 'visible' : 'hidden'}`}><Dashboard state={state} notify={notify} active={page === 'dashboard'}/></div>{page !== 'dashboard'&&view}{modeLocked&&page!=='profiles'&&<OnboardModeLock onOpenProfiles={()=>setPage('profiles')}/>}</>}
      </main>
